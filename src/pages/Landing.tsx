import { ArrowRight, BadgeCheck, CheckCircle2, Lock, Network, Orbit } from "lucide-react";

interface Props {
  onLaunch: () => void;
}

function Header({ onLaunch }: Props) {
  return (
    <header className="fixed top-0 w-full z-50 bg-background border-b-2 border-primary">
      <div className="flex justify-between items-center h-16 px-5 max-w-7xl mx-auto">
        ...
        <button onClick={onLaunch} className="flex items-center gap-1.5 bg-primary text-white font-epilogue font-bold text-sm px-3 py-1.5 border-2 border-primary hard-shadow-sm lift-interaction active:translate-x-0 active:translate-y-0 text-center">
          Launch App
  ...
  return (
    <section className="px-5 pt-20 pb-16 flex flex-col gap-6 max-w-3xl mx-auto">
      ...
      <div className="flex flex-col gap-3 mt-4">
        <button onClick={onLaunch} className="bg-accent-yellow text-primary font-epilogue text-xl md:text-2xl font-bold py-6 border-2 border-primary hard-shadow-md lift-interaction">
          Start Thinking — Free
        </button>
        <div className="flex items-center gap-2 font-epilogue font-bold text-xs md:text-sm uppercase tracking-widest text-primary mt-2">
          ...
          Local-first. No account. Runs in your browser.
        </div>
      </div>
    </section>
  );
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="flex flex-col md:grid md:grid-cols-3 gap-8 md:gap-6 lg:gap-8 mt-4">
          ...
          <div className="bg-surface border-2 border-primary hard-shadow-md p-8 relative flex flex-col gap-3">
            ...
            <h3 className="font-epilogue text-2xl font-bold">Auto-Connect</h3>
            <p className="font-manrope text-base text-on-surface-variant">Our engine links every thought. It sees the semantic thread between a meeting note and a midnight idea.</p>
          </div>
...
function Features({ onLaunch }: Props) {
  return (
    <section className="px-5 py-20 flex flex-col bg-surface-container border-t-2 border-primary">
      <div className="max-w-4xl mx-auto w-full flex flex-col gap-12">
        ...
        <div className="bg-surface border-2 border-primary p-6 md:p-10 flex flex-col gap-4 md:gap-6 hard-shadow-md">
          ...
          <button onClick={onLaunch} className="self-start flex items-center gap-1.5 bg-primary text-white font-epilogue font-bold text-sm px-4 py-2 border-2 border-primary hard-shadow-sm lift-interaction">
...
function Pricing({ onLaunch }: Props) {
  return (
    <section className="px-5 py-24 flex flex-col items-center border-t-2 border-primary bg-background">
      <div className="w-full max-w-lg bg-accent-yellow border-2 border-primary hard-shadow-lg p-10 md:p-14 flex flex-col gap-8 items-center text-center">
        ...
        <h2 className="font-epilogue text-[80px] md:text-[96px] font-bold leading-none tracking-tight">$0</h2>
        <span className="font-epilogue text-2xl font-bold">Free &amp; local-first</span>
        ...
        <div className="flex flex-col gap-4 w-full text-left py-8">
          <div className="flex items-center gap-3 border-b-2 border-primary/20 pb-4">
            ...
            <p className="font-manrope text-lg font-bold">Unlimited thoughts &amp; connections</p>
          </div>
          <div className="flex items-center gap-3 border-b-2 border-primary/20 pb-4">
            <CheckCircle2 className="w-6 h-6 shrink-0 text-primary" strokeWidth={2.5} />
            <p className="font-manrope text-lg font-bold">Bring your own Gemini key for AI</p>
          </div>
          <div className="flex items-center gap-3 border-b-2 border-primary/20 pb-4">
            <CheckCircle2 className="w-6 h-6 shrink-0 text-primary" strokeWidth={2.5}/>
            <p className="font-manrope text-lg font-bold">Your data never leaves the browser</p>
          </div>
        </div>

        <button onClick={onLaunch} className="w-full bg-primary text-white font-epilogue text-2xl font-bold py-6 border-2 border-primary hard-shadow-md lift-interaction">
          Open MindGraph
        </button>
      </div>
    </section>
